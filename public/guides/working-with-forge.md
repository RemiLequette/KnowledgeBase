...
For structured types with dynamic block structures, you may need to re-describe after the file changes:
```
forge_describe(fal, force=true)   → resets the described flag, re-reads the artifact structure
```
...
Always free — no describe or brand required. Returns branded FALs with their type. **This is the only tool that issues valid FALs.** Use it to navigate and discover artifact addresses.
...
forge_describe(fal, force=true)  → re-describe, reset the unlock flag
```
...
Requires prior `forge_describe` for the type (RTFM) and a branded FAL (Brand). For plain-text types (`md`, `js`, `json`, `html`, `css`, `txt`), always omit the block argument — these types have no block structure.
...
Requires prior `forge_describe` for the type (RTFM) and a branded FAL (Brand). For plain-text types, always omit the block argument.
